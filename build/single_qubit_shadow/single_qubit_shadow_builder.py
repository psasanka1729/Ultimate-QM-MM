...
#Do the git-ing
cmd='git commit -a -m "Commit before run single_qubit_shadow" > /dev/null'
print(cmd)
...
open('single_qubit_shadow.log','w').write(logstr)

#Setup the versionmap and qsub files
vmap_file=open('versionmap.dat','w')
vmap_file.write('vnum\tL\n')

task_file=open('single_qubit_shadow.task','w')
template_file='single_qubit_shadow.template'
...
for L in xrange(256):
	...
	task_file.write('bash single_qubit_shadow_'+str(vnum)+'.qsub\n')
	fout.write(contents)
	fout.close()
...
contents=open('single_qubit_shadow.sbatch.template','r').read()
...
open('single_qubit_shadow.sbatch','w').write(contents)
